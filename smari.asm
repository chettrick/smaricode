...
; port A (address 00H):
;	bit 7	-- I2C bus serial data bi-directional
...
; then jump to this address.  Initially, 0FFEEH and 0FFEFH
; are set to 0066H.  So RST 38 will have the same effect
; as software break.  By changing the content of 0FFEEH
; and 0FFEFH, the user can define his or her own service
; routine.
; The next three instructions push the contents of 0FFEEH
; and 0FFEFH to stack without changing any registers.
...
;   6=WT	;Expect parameters for 'USBWR' function.
;   7=RT	;Expect parameters for 'USBRD' function.
...
KWT:
;Branched by table KFUN.
;Executed when 'USB WR' key is pressed.

KRT:
;Branched by table KFUN.
;Executed when 'USB RD' key is pressed.
...
LEAD1	CALL	PERIOD		;The return of PERIOD
	...
	JR	NZ, LEAD1	;Wait for 1000 periods.
				...
				;cycles (1 second).
LEAD2	CALL	PERIOD
	JR	NC, LEAD2	;Wait for all leading sync to
				;pass over.
...
FILEDP	CALL	SCAN1
	...
	OUT	(SEG7), A	;If filename is found
				;Display '------'.
...
OUT0:				;2kHz 8 cycles, 1kHz 2 cycles.
	LD	L, ZERO_2K
	CALL	TONE2K
	LD	L, ZERO_1K
	JR	BITEND
;
OUT1:				;2kHz 4 cycles, 1kHz 4 cycles.
	LD	L, ONE_2K
	CALL	TONE2K
	LD	L, ONE_1K
BITEND	CALL	TONE1K
...
;
;***********************************************************
; Function: RAM 8000-FFFF self-check.
; Input: none
; Output: none
